import { configUtils, getRevisionResultArray, } from "../../lib/transformWalkResultsUtils";
const getGlobalScoreHistory = (results) => results.map(({ commitDate, totalScore }) => ({
    date: commitDate,
    score: totalScore,
}));
const getRuleScoreHistory = (results, ruleId, startDate) => results
    .map(({ commitDate, rules }) => ({
    date: commitDate,
    score: rules[ruleId]?.score || 0,
}))
    .filter(({ date }) => date >= startDate);
const getRuleEstimation = (scoreHistory) => {
    ...
    const daysToTime = (days) => days * 1000 * 60 * 60 * 24;
    ...
    const estimatedendDate = daysToReachZero > 0
...
const getDateRulesDatas = ({ results, rules, getConfigRuleById, }) => {
    const rulesObjInit = rules.reduce((acc, ruleId) => {
        const ruleStartTrackingDate = getConfigRuleById(ruleId)?.startTrackingDate ?? "1990-01-01";
        const scoreHistory = getRuleScoreHistory(results, ruleId, new Date(ruleStartTrackingDate).getTime());
        acc[ruleId] = {
            id: ruleId,
...
export function estimationAdapter(data) {
    ...
    const currentScore = fileRulesScoresArray[fileRulesScoresArray.length - 1].totalScore;
