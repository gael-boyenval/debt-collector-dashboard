export const getConfigTags = (config) => {
    const getTagsFromRules = (rules) => rules.reduce((acc, rule) => {
...
export const getFileRulesDates = (config) => {
    const getDatesFromRules = (rules) => rules.reduce((acc, rule) => {
...
export const getRevAdoptionRulesScores = (adoptionRulesId, results) => {
    const adoptionRulesScores = adoptionRulesId.reduce((acc, rule) => {
        acc[rule] = 0;
        return acc;
    }, {});
    Object.values(results).forEach((fileResults) => {
...
export const getRevisionResultArray = ({ config, results, }) => {
    const allFileRulesIdInConfig = getFileRulesIds(config);
    const revisionResultsArr = results.map(({ rev, results }) => {
        const fileRulesScores = getRevFileRulesScores(allFileRulesIdInConfig, results);
        const totalScore = Object.values(fileRulesScores).reduce((acc, rule) => acc + rule.score, 0);
        return {
            commit: rev.hash,
...
const getTrackAdoptionMetrics = (config) => {
...
export const configUtils = (config) => {
    return {
        ...
        getConfigRuleById: (ruleId) => getConfigRuleById({ config, ruleId, key: "fileRules" }),
        getAdoptionRulesById: (ruleId) => getConfigRuleById({ config, ruleId, key: "adoptionRules" }),
